...
import './FitnessLeft.css';
import { Link } from 'react-router-dom';

function FitnessLeft() {
    const [data, setData] = useState([]);

  useEffect(() => {
    fetch('http://localhost:5001/fitness')
    ...
    <div className='BollywoodComp'>
        ...
        <div className='mainBox' key={movie.id}>
           <div className="main-imgbox">
            <Link to={`/article/${movie.id}`}><img className='main-img' src={movie.image} alt={movie.name} /></Link>
           </div>
